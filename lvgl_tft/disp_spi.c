 ...
 *      DEFINES
 *********************/
#define SPI_TRANSACTION_POOL_SIZE 10	/* maximum number of DMA transactions simultaneously in-flight */
 ...
 *  STATIC PROTOTYPES
 **********************/
static void IRAM_ATTR spi_prepare (spi_transaction_t *trans);
...
{
    static spi_transaction_ext_t *tpriv = NULL;
    ...
    if (flags & (DISP_SPI_SEND_POLLING | DISP_SPI_SEND_SYNCHRONOUS)) {
        if (tpriv == NULL) {
            tpriv = (spi_transaction_ext_t*)heap_caps_malloc(sizeof(spi_transaction_ext_t), MALLOC_CAP_DMA);
            assert(tpriv != NULL);
        }
        t = tpriv;
    } else {
        if (xQueueReceive(TransactionPool, &t, 0) != pdTRUE) {
            ESP_ERROR_CHECK(spi_device_get_trans_result(spi, (spi_transaction_t **)&t, portMAX_DELAY));
        }
    }
    bzero(t, sizeof(*t));
    struct spi_transaction_t *tbase = &t->base;

    /* transaction length is in bits */
    tbase->length = length * 8;

    if (length <= 4 && data != NULL) {
        tbase->flags = SPI_TRANS_USE_TXDATA;
        memcpy(tbase->tx_data, data, length);
    } else {
        tbase->tx_buffer = data;
    }
...
#if defined(DISP_SPI_HALF_DUPLEX)
		tbase->rxlength = tbase->length;
		tbase->length = 0;	/* no MOSI phase in half-duplex reads */
#else
		tbase->rxlength = 0; /* in full-duplex mode, zero means same as tx length */
#endif
    }
    ...
    if (flags & DISP_SPI_SEND_POLLING) {
        ...
        if (spi_device_queue_trans(spi, (spi_transaction_t *)t, portMAX_DELAY) != ESP_OK) {
			xQueueSend(TransactionPool, &t, portMAX_DELAY);	/* send failed transaction back to the pool to be reused */
        }
    }
}
...
static void IRAM_ATTR spi_prepare(spi_transaction_t *trans)
{
    const disp_spi_send_flag_t flags = (disp_spi_send_flag_t) trans->user;

    if (flags & DISP_SPI_RESET_CD) {
        gpio_set_level(CONFIG_LV_DISP_PIN_DC, 0);	 /* Command mode */
    } else if (flags & DISP_SPI_SET_CD) {
        gpio_set_level(CONFIG_LV_DISP_PIN_DC, 1);	 /* Data mode */
    }
}
